...
**API/config/data model:** Uses direct `@revolist/pivot`, `@revolist/gantt`, and `@revolist/scheduler` trial aliases, with `@revolist/revogrid-pro` for Tree and shared plugins. Shared RevoGrid defaults live in `src/shared/grid.ts`; each example owns its data/config in `src/examples/*`.
...
- E2E: `pnpm test:e2e` starts the Vite trial app, verifies the Tree owner/status dropdown menus, and checks the Pivot, Gantt, and Scheduler examples render through the header switch.
